...
Scan the project directory provided in the prompt and produce a JSON inventory. You will accomplish this in two phases: first, invoke the `understandable` walker to perform all deterministic file scanning; second, review the walker's results and add a human-readable project description.

---

## Phase 1 -- Discovery via the `understandable` walker

The `understandable` binary already implements a deterministic project
walker that discovers all project files (including non-code files
like configs, docs, and infrastructure), detects languages and
frameworks, counts lines, and pre-resolves project-internal imports.
You do **not** write a Node.js or Python script for this — invoke the
binary as documented under "Executing the Walker" below.

The sections that follow describe **what the walker produces** so you
can validate its output. They are reference material, not
implementation instructions.

### Walker Contract

1. **Input:** the project root directory passed via `--path`.
2. **Output:** structured JSON on stdout, redirected to a file under
   `$PROJECT_ROOT/.understandable/tmp/`.
3. **Exit 0** on success.
4. **Exit 1** on fatal error (cannot access directory, etc.). The
...
You do not need to implement this filtering yourself.
`understandable analyze --plan-only` (executed below) walks the
project with the same gitignore-aware engine the rest of the binary
uses. It honours both `.gitignore` and `.understandignore` natively
and supports `!` negation patterns (e.g. `!dist/` force-includes
`dist/` files).

The walker also reports `filteredByIgnore` in its output JSON, which
is the count of files removed by `.understandignore` patterns beyond
the baseline gitignore filter. Surface that number in the final scan
result as-is — do not re-run any custom filtering.
...
Output format in the walker result:
```json
"importMap": {
...
### Walker Output Format

The walker writes this exact JSON structure to its stdout:
...
- `scriptCompleted` (boolean) -- always `true` when the walker finishes normally
...
### Executing the Walker

Do NOT write a custom Node.js or Python discovery script. The
`understandable` binary already enumerates files with the
gitignore-aware walker, detects languages and frameworks, counts
lines, and pre-resolves project-internal imports. Invoke the
plan-only mode of the analyser to run all of Steps 1–9 above in a
single deterministic call. `$PROJECT_ROOT` is the project root
provided in your dispatch prompt:

```bash
understandable analyze --incremental --plan-only --path "$PROJECT_ROOT" \
  > "$PROJECT_ROOT/.understandable/tmp/ua-scan-results.json"
```

`--plan-only` is gated on `--incremental` in clap (`requires =
"incremental"`). Plan-only mode still walks the project with the
gitignore-aware engine and emits the full Walker Output JSON, but
skips persistence — exactly what we want for a dry scan.

The binary writes the same JSON shape documented in **Walker Output
Format** above (including `name`, `languages`, `frameworks`, `files`
with `fileCategory`, `totalFiles`, `filteredByIgnore`,
`estimatedComplexity`, and the full `importMap`). It also includes
`rawDescription` and `readmeHead` so you can synthesise the final
`description` field in Phase 2.

If the binary exits with a non-zero code, read stderr and surface the
error. Do NOT attempt to fall back to a hand-written script — the
binary is the sole scan path.

---

## Phase 2 -- Description and Final Assembly

After the walker completes, read `$PROJECT_ROOT/.understandable/tmp/ua-scan-results.json`. Do NOT re-run file discovery commands or re-count lines -- trust the walker's results entirely.

**IMPORTANT:** The final output must NOT contain the `scriptCompleted`, `rawDescription`, or `readmeHead` fields. These are intermediate walker fields only. Strip them when assembling the final JSON. All other fields — including `importMap` — MUST be preserved exactly as output by the walker.
...
- NEVER invent or guess file paths. Every `path` in the `files` array must come from the walker's file discovery, which uses the `understandable` binary's gitignore-aware traversal.
...
- Trust the walker's output for all structural data. Your only contribution is the `description` field.

## Writing Results

After producing the final JSON:

1. Create the output directory: `mkdir -p <project-root>/.understandable/intermediate`
2. Write the JSON to: `<project-root>/.understandable/intermediate/scan-result.json`
