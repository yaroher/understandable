...
Given a list of file nodes (with paths, summaries, tags, and node types) and import edges, identify 3-10 logical architecture layers and assign every file node to exactly one layer. You will accomplish this in two phases: first, export the persisted graph via `understandable export --kind codebase` and compute structural patterns from the resulting JSON; second, use those structural insights to make semantic layer assignments.

---

## Phase 1 -- Structural Analysis via the `understandable` binary

You do **not** write a Node.js or Python script for this. The
`understandable` binary already exposes the full structural graph
(file/config/document/service/pipeline/table/schema/resource/endpoint
nodes plus every edge type) via its `export` subcommand. Read the
exported JSON and compute the structural patterns documented below
in-process — there is no separate analysis script to author.

### Walker Contract

1. **Input:** the persisted graph at `.understandable/graph.tar.zst`.
   Export it with `understandable export --kind codebase` (see
   "Executing the Export" below). The exported JSON contains:
   ```json
   {
     "fileNodes": [
   ...
   ```
2. **Output:** the same `fileNodes`, `importEdges`, and `allEdges`
   shape as documented in your dispatch prompt — derived directly
   from `understandable export --kind codebase`.

### Structural Patterns to Compute
...
### Required Computed Output Format
...
Export the persisted graph as JSON. The binary writes the full
node/edge set with every prefix (`file:`, `config:`, `document:`,
`service:`, `pipeline:`, `table:`, `schema:`, `resource:`,
`endpoint:`) plus all edge categories (`imports`, `configures`,
`documents`, `deploys`, `triggers`, etc.) — that's the same input
shape the legacy script consumed.

```bash
understandable export --kind codebase --path "$PROJECT_ROOT" \
  > "$PROJECT_ROOT/.understandable/tmp/ua-arch-input.json"
```

Then compute the structural patterns above (directory groups, node
type groups, cross-category edges, inter-group imports, intra-group
density, pattern matches, deployment topology, data pipeline, doc
coverage, dependency direction, file fan-in/fan-out) directly from
that JSON. Persist your computed result to
`$PROJECT_ROOT/.understandable/tmp/ua-arch-results.json` so Phase 2
can read it.

If `understandable export` exits non-zero, read stderr and surface
the error. Do NOT fall back to a hand-written analysis script — the
binary is the sole graph-export path.

---

## Phase 2 -- Semantic Layer Assignment

After Phase 1 completes, read `$PROJECT_ROOT/.understandable/tmp/ua-arch-results.json`. Use the structural analysis as the primary input for your layer decisions. Do NOT re-read source files or re-analyze imports -- trust the computed results entirely.

### Step 1 -- Evaluate Directory Groups as Layer Candidates

For each directory group from the computed output:
...
Choose layers based on the project's actual architecture, informed by the computed structural data. Common patterns include:
...
**Cross-check:** The sum of all `nodeIds` array lengths across all layers MUST equal the total number of file nodes from the input (`fileStats.totalFileNodes` from the computed output).
...
- Trust the computed structural analysis. Do NOT re-read source files or re-count imports. The adjacency data, density calculations, and pattern matches are deterministic and reliable.
- If the computed output produces empty directory groups or groups with zero files, skip them — do not create empty layers.

## Writing Results

After producing the JSON:

1. Write the JSON array to: `<project-root>/.understandable/intermediate/layers.json`
