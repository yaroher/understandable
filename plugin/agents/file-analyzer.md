...
Execute the pre-built structural extraction script bundled with the understandable plugin. This script uses tree-sitter for code files and specialized parsers for non-code files, providing deterministic, high-quality structural extraction without writing any ad-hoc scripts.
...
```bash
cat > $PROJECT_ROOT/.understandable/tmp/ua-file-analyzer-input-<batchIndex>.json << 'ENDJSON'
{
...
### Step 2 — Execute the bundled extraction binary

Call the `understandable extract` subcommand. It reads the JSON batch
input you wrote in Step 1 and writes the JSON output schema documented
in Step 3.

```bash
understandable extract \
  --batch $PROJECT_ROOT/.understandable/tmp/ua-file-analyzer-input-<batchIndex>.json \
  --out   $PROJECT_ROOT/.understandable/tmp/ua-file-extract-results-<batchIndex>.json
```

If the binary exits non-zero, read stderr and report the error. Do NOT attempt to write a manual extraction script as fallback — `understandable extract` is the sole extraction path.

### Step 3 — Read the extraction results

Read `$PROJECT_ROOT/.understandable/tmp/ua-file-extract-results-<batchIndex>.json`. The output format is:
...
**Supported file categories:** `understandable extract` handles all file categories — `code` (tier 1: TypeScript, JavaScript, Python, Go, Rust, Java, Ruby, PHP, C/C++, C#), `config`, `docs`, `infra`, `data`, `script`, and `markup`. For tier-2 languages without full structural extraction (Swift, Lua, Zig, Dart, Scala, Haskell, OCaml, etc.), the binary outputs basic metrics with empty structural data — use your judgment to supplement from source file reading if needed.

---

## Phase 2 -- Semantic Analysis

After the script completes, read `$PROJECT_ROOT/.understandable/tmp/ua-file-extract-results-<batchIndex>.json`. Use these structured results as the foundation for your analysis. Do NOT re-read the source files unless the script skipped a file or you need to understand a specific pattern that the script could not capture.
...
1. Write the JSON to: `<project-root>/.understandable/intermediate/batch-<batchIndex>.json`
